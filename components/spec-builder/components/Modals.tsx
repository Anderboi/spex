        ...
        <div className="py-6 px-6">
          ...
          {rooms.length > 0 && (
            <div className="mt-3 bg-bg-gold border border-border-gold rounded-[13px] py-3 px-4">
              ...
              <div className="flex flex-wrap gap-2 mt-2">{rooms.map((r: string) => <span key={r} className="text-[12.5px] font-medium bg-bg-white border border-border-gold text-fg-gold-text rounded-full py-1 px-3">{r}</span>)}</div>
            </div>
          )}
          {rooms.length === 0 && (
            <div className="mt-6 bg-bg-card border border-border rounded-[13px] py-3 px-4 text-[13px] text-fg-muted">Назначений в помещениях нет — марку можно безопасно удалить.</div>
          )}

          {!replacing ? (
            <div className="flex flex-col gap-2 mt-4">
              <div onClick={onDeleteFull} className="border border-border-red-light bg-bg-delete rounded-[14px] p-4 px-4 cursor-pointer">
                <div className="text-[15.5px] font-semibold text-fg-red">🗑 Удалить полностью</div>
                <div className="text-[13px] text-fg-light-text mt-1 leading-[1.45]">Марка удаляется безвозвратно. Последующие марки с префиксом «{prefixOf(item.code)}» сместятся вверх.</div>
              </div>
