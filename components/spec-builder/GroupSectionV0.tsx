  ...
  return (
    <div className="mt-6">
      ...
      {group.showTable && group.open && (
        <>
          <div className="grid grid-cols-[38px_46px_58px_minmax(150px,1.5fr)_1.1fr_90px_104px_122px_140px_86px] gap-3.5 py-3 px-1 pb-2 font-mono text-[9.5px] tracking-widest uppercase text-fg-dim">
          ...
          <div onClick={() => onAddPlaceholder(group.type)} className="flex items-center gap-3 py-3 px-1 border-t border-dashed border-fg-dash text-fg-muted cursor-pointer font-sans text-[14px] font-semibold">
            <span className="size-6 rounded-[7px] border-[1.5px] border-dashed border-fg-dash-dots flex items-center justify-center text-[16px] leading-none">+</span> Добавить позицию
          </div>
        </>
      )}
      ...
      {group.showCards && group.open && group.items.map((it: SpecItem) => (
        it.placeholder ? (
          <div key={it.id} onClick={() => onOpenFill(it)} className="bg-transparent border-[1.5px] border-dashed border-fg-dash rounded-[16px] p-4 mt-[10px] cursor-pointer flex items-center gap-3">
            <div className="size-12 rounded-[9px] border-[1.5px] border-dashed border-fg-dash-dots flex items-center justify-center text-fg-icon text-[24px] leading-none flex-none">+</div>
            <div className="flex-1 min-w-0"><div className="text-[16px] font-medium italic text-fg-secondary">Материал не выбран</div><div className="font-mono text-[10px] text-fg-secondary mt-1">{it.code} · нажмите, чтобы заполнить</div></div>
          ...
          <div key={it.id} className="bg-bg-card border border-border rounded-[16px] p-[14px] mt-[10px]">
            <div className="flex gap-3">
              <div onClick={() => onOpen(it.id)} className="size-[54px] rounded-[9px] border border-border-placeholder flex-none cursor-pointer [background:repeating-linear-gradient(135deg,#e9e4d9,#e9e4d9_5px,#f0ebe1_5px,#f0ebe1_10px)]"></div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-bg-clear">
      ...
      {group.showCards && group.open && (
        <div onClick={() => onAddPlaceholder(group.type)} className="flex items-center gap-3 p-4 mt-[10px] border-[1.5px] border-dashed border-fg-dash rounded-[16px] text-fg-muted cursor-pointer font-sans text-[14px] font-semibold">
