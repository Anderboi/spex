...
/**
 * Общий селект фильтров библиотеки материалов: производитель, статус, сортировка.
 *
 * Триггер, попап и подписи пунктов обязаны совпадать у всех трёх контролов,
 * поэтому стили и `items` живут здесь, а не дублируются по файлам. `items`
 * нужен base-ui, чтобы `SelectValue` показывал подпись выбранного пункта:
 * у пункта «без фильтра» значение `null`, и без `items` триггер показал бы
 * пустоту вместо подписи.
 */
export function MaterialsFilterSelect<T extends string>({
  ariaLabel,
  placeholder,
  options,
  value,
